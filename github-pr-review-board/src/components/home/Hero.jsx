...
export default function Hero() {
  ...
  const openedFromInitButton = useRef(false);
  ...
  const handleInitClick = () => {
    if (repoData) {
      ...
      handleModal();
      openedFromInitButton.current = true;
    }
  };

  useEffect(() => {
    if (repoData && openedFromInitButton.current) {
      navigate("open-prs");
      openedFromInitButton.current = false;
    }
  }, [repoData]);
  ...
  return (
    <section className='flex flex-col items-center justify-center relative py-15 px-3 bg-background bg-[linear-gradient(rgba(0,255,255,0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(0,255,255,0.05)_1px,transparent_1px)] bg-[length:50px_50px] overflow-hidden'>
      ...
      <div className='relative text-white flex flex-col items-center justify-center gap-8'>
        ...
        <p className='bg-[#1E232D33] p-5 text-sm border-l-4 border-brand-primary leading-relaxed text-text w-7/8 md:max-w-[550px]'>
        ...
        <div className='flex flex-col gap-10'>
          <button
            className='group py-4 px-6 cursor-pointer flex text-sm  items-center bg-background justify-center gap-2 border tracking-widest uppercase text-brand-primary border-brand-
          hover:bg-brand-primary hover:text-background hover:border-background
          ...
          <div className='flex items-center justify-center gap-6'>
            <div className='w-[8px] h-[8px] bg-brand-primary cursor-pointer p-[4px]'></div>
            <div className='w-[8px] h-[8px] bg-brand cursor-pointer p-[4px]'></div>
            <div className='w-[8px] h-[8px] bg-brand-secondary cursor-pointer p-[4px]'></div>
          </div>
        </div>
      </div>
