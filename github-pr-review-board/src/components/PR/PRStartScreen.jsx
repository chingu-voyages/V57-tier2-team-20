      ...
      {!repoData ? (
        <div className="relative flex flex-col gap-8 md:w-3/4 items-center justify-center bg-card border border-brand-primary/20 py-12 px-6 h-100">
          ...
          <div className="flex flex-col justify-center items-center gap-3">
            <h1 className="capitalize text-5xl font-bold text-white text-center">
              open PRs dashboard
            </h1>
            <p className="text-text text-sm text-center">
              Initialize a repository from the home page to view open pull
            </p>
          </div>
          <button
            className="flex items-center justify-center gap-2 bg-brand-primary px-4 py-2 text-sm cursor-pointer"
            onClick={handleModal}
          >
            <Icon
        ...
        <OpenedPRs org={newPrDetails.orgName} repo={newPrDetails.repoName} />
      )}
      {/* Popup Modal */}
      {/* {showModal && <Modal />} */}
      {/* <Modal /> */}
